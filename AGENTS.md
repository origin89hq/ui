...
`packages/react` holds React components. The root Swift package builds `platforms/swiftui`; `platforms/compose` is the Android library. Brand assets come from the released `@origin89/brand` package. Run `just check`, `just swift-check`, and `just android-check` before publishing. Preserve unknown and stale states; these libraries do not send equipment commands.
